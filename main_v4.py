...
try:
    # LCD ST7789
    serial_lcd = luma_spi(port=0, device=0, gpio_DC=24, gpio_RST=25, baudrate=80000000)
    device = st7789(serial_lcd, width=WIDTH, height=HEIGHT, rotate=0, framebuffer="diff_to_previous")
...
items = [   
                ...
                ("Camera", "☠", "#fab387"), ("Weather", "☁", "#89dceb"), ("Cài Đặt", "⚙", "#cba6f7")]
...
CITIES_DATA = [
    {"display_name": "Hà Nội", "lat": 21.0245, "lon": 105.8412},
    {"display_name": "Đà Nẵng", "lat": 16.068, "lon": 108.212},
    {"display_name": "Hồ Chí Minh", "lat": 10.762622, "lon": 106.660172}
]
...
class PiMediaCenter:
    def __init__(self):
        ...
        self.scan_interval = 100  # Seconds

        # Sửa thêm cho mật khẩu WiFi
        self.hide_password = False  # Nếu True, hiển thị '*' thay vì ký tự thật
    ...
    def draw_email_ui(self, draw):
        ...
        # Hiển thị người nhận
        recipient_name = recipient_email[self.current_email_index].split('@')[0]
    ...
    def draw_password_input(self, draw):
        ...
        # Hiển thị mật khẩu (ẩn nếu self.hide_password = True)
        if self.hide_password:
            display_text = '*' * len(self.wifi_password) + "_"
        else:
    ...
    def draw_weather_ui(self, draw):
        """Giao diện hiển thị thời tiết"""
        draw.rectangle((0, 0, WIDTH, HEIGHT), fill=BG_COLOR)
        self.draw_status_bar(draw)

        if self.weather_error:
            draw.text((10, 100), self.weather_error, fill=WARN_COLOR, font=font_md)
        elif not self.weather_data:
            draw.text((10, 100), "Đang tải dữ liệu...", fill=TEXT_COLOR, font=font_md)
        else:
            city = CITIES_DATA[current_city_index]["display_name"]
            temp = self.weather_data.get("main", {}).get("temp", "N/A")
            description = self.weather_data.get("weather", [{}])[0].get("description", "N/A").capitalize()
            humidity = self.weather_data.get("main", {}).get("humidity", "N/A")
            wind_speed = self.weather_data.get("wind", {}).get("speed", "N/A")
            feels_like = self.weather_data.get("main", {}).get("feels_like", "N/A")

            # Vẽ tiêu đề
            draw.text((10, 30), f"Thời tiết tại {city}", fill=ACCENT_COLOR, font=font_md)

            # Vẽ thông tin chính
            y_pos = 60
            draw.text((10, y_pos), f"Nhiệt độ: {temp}°C", fill=TEXT_COLOR, font=font_md)
            y_pos += 20
            draw.text((10, y_pos), f"Cảm giác: {feels_like}°C", fill=TEXT_COLOR, font=font_md)
            y_pos += 20
            draw.text((10, y_pos), f"Mô tả: {description}", fill=TEXT_COLOR, font=font_md)
            y_pos += 20
            draw.text((10, y_pos), f"Độ ẩm: {humidity}%", fill=TEXT_COLOR, font=font_md)
            y_pos += 20
            draw.text((10, y_pos), f"Gió: {wind_speed} m/s", fill=TEXT_COLOR, font=font_md)

        # Nút điều hướng
        btn_y = 205
        self.draw_button(draw, 10, btn_y, 90, 30, "◀ Trước")
        self.draw_button(draw, 115, btn_y, 90, 30, "Cập nhật", bg_color=SUCCESS_COLOR, text_color="black")
        self.draw_button(draw, 220, btn_y, 90, 30, "Sau ▶")
    ...
    def scan_wifi(self):
        ...
        self.state = "WIFI_LIST"  # SỬA: Chuyển state để hiển thị danh sách
        if not self.wifi_networks:
            img = Image.new("RGB", (WIDTH, HEIGHT), "black")
            d = ImageDraw.Draw(img)
            d.text((80, 100), "Không tìm thấy mạng WiFi!", fill=WARN_COLOR, font=font_md)
            device.display(img)
            time.sleep(2)
        self.render()
    ...
    def scan_bt(self):
        ...
        self.state = "BT_LIST"  # SỬA: Chuyển state để hiển thị danh sách
        if not self.bt_devices:
            img = Image.new("RGB", (WIDTH, HEIGHT), "black")
            d = ImageDraw.Draw(img)
            d.text((80, 100), "Không tìm thấy thiết bị BT!", fill=WARN_COLOR, font=font_md)
            device.display(img)
            time.sleep(2)
        self.render()
    ...
    def find_keyboard_device(self):
        print("dang tim kiem ban phim")
        candidates = []
        for event in os.listdir('/dev/input/'):
            if event.startswith('event'):
                try:
                    dev = InputDevice(f'/dev/input/{event}')
                    print(f"Device: {dev.name} at {dev.path}")
                    if ecodes.EV_KEY in dev.capabilities() and ('keyboard' in dev.name.lower() or 'hid' in dev.name.lower()):
                        print(f"tim thay ban phim: {dev.name}")
        ...
        if candidates:
            for dev in candidates:
                if 'keyboard' in dev.name.lower() and 'control' not in dev.name.lower() and 'hdmi' not in dev.name.lower():
                    print(f"chon ban phim: {dev.name}")
                    return dev
            for dev in candidates:
                if 'keyboard' in dev.name.lower():
                    print(f"chon fallback: {dev.name}")
                    return dev
    ...
    def keyboard_input_loop(self):
        """Luồng đọc sự kiện bàn phím từ evdev"""
        while self.state == "PLAYING_GAME" and self.keyboard_device:
            for event in self.keyboard_device.read_loop():
                if event.type == ecodes.EV_KEY:
                    key_code = event.code
                    press_event = WindowEvent.PRESS_ARROW_UP  # Mặc định
                    release_event = WindowEvent.RELEASE_ARROW_UP
                    
                    # Map key codes (cần điều chỉnh theo bàn phím cụ thể)
                    ...
                    elif key_code == ecodes.KEY_LEFTSHIFT or key_code == ecodes.KEY_RIGHTSHIFT:
    ...
    def run_game(self):
        ...
        try:
            ...
            # 2. Thiết lập Input bàn phím với evdev
            self.keyboard_device = self.find_keyboard_device()
            if not self.keyboard_device:
                print("Không tìm thấy bàn phím USB!")
                self.state = "GAMES"
                self.render()
                return

            # Chạy luồng đọc bàn phím
            ...
            if self.keyboard_thread and self.keyboard_thread.is_alive():
                # evdev không có stop, chỉ cần thay đổi state để loop thoát
                pass
    ...
    def apply_tone_mark(self, word):
        """Bổ sung dấu thanh điệu vào nguyên âm cuối cùng của từ. Thêm kiểm tra an toàn."""
        if not word:
            return ""
        
        # Tìm nguyên âm cuối cùng (chỉ cycle nếu có nguyên âm tiếng Việt)
        vowel_positions = []
        for i, char in enumerate(word):
            for variants in TONE_MARKERS.values():
                if char in variants:
                    vowel_positions.append(i)
                    break
        
        if not vowel_positions:
            return word  # Không cycle nếu không có nguyên âm phù hợp (số/đặc biệt)
    ...
    def apply_tone_mark_on_last_word_wifi(self):
        """Áp dụng cycle dấu cho từ cuối cùng trong password WiFi, chỉ nếu kb_mode == 'abc'."""
        if self.kb_mode != "abc":  # Bỏ qua cycle nếu đang ở mode số/đặc biệt
            return
...
# --- USB CAMERA INIT ---
    ...
    def fetch_weather_data(self):
        ...
        city_data = CITIES_DATA[current_city_index]
        params = {
            "lat": city_data["lat"],
            "lon": city_data["lon"],
            "appid": API_KEYS[current_key_index],
            "units": UNITS
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                current_key_index = (current_key_index + 1) % (MAX_KEY_INDEX + 1)
                if current_key_index == 0:  # Đã thử hết key
                    self.weather_error = "Tất cả API key không hợp lệ!"
                else:
                    return self.fetch_weather_data()  # Thử key tiếp theo
            else:
                self.weather_error = f"Lỗi {response.status_code}: {response.text}"
        except Exception as e:
            self.weather_error = f"Lỗi kết nối: {str(e)}"

    def update_weather(self):
        """Cập nhật thời tiết nếu cần"""
        if time.time() - self.last_weather_update > self.weather_update_interval or not self.weather_data:
    ...
    def handle_touch(self, x, y):
            ...
            for key in row:
                ...
                if curr_x <= x <= curr_x + w:
                    if key == "Shift":
                        ...
                        if self.wifi_password and len(self.wifi_password) >= 8:  # Kiểm tra độ dài tối thiểu
                            ...
                            # Hiển thị lỗi nếu mật khẩu rỗng hoặc ngắn
                            img = Image.new("RGB", (WIDTH, HEIGHT), "black")
                            d = ImageDraw.Draw(img)
                            d.text((80, 100), "Mật khẩu quá ngắn!", fill="red", font=font_md)
                            device.display(img)
                            time.sleep(2)
                            self.render()
                    else:
        ...
        # --- EMAIL UI (Xử lý chạm bàn phím ảo) ---
            ...
            if y < 50:
                self.current_email_index = (self.current_email_index + 1) % len(recipient_email)
                self.render()
                return
        ...
        # --- WEATHER UI ---
            ...
            # Nav Buttons
            if y > 200:
                if x < 100:  # Trước (thay đổi thành phố trước)
                    temp = current_city_index
                    current_city_index = (temp - 1) % len(CITIES_DATA)
                    self.fetch_weather_data()
                elif x > 220:  # Sau (thay đổi thành phố sau)
                    temp = current_city_index
                    current_city_index = (temp + 1) % len(CITIES_DATA)
                    self.fetch_weather_data()
                else:  # Cập nhật (refresh dữ liệu)
                    self.fetch_weather_data()
                self.render()
